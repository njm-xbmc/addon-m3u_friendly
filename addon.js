...
function decodeConfig(str) {
  try {
    ...
    if (!Array.isArray(config.m3uUrls) || !config.m3uUrls.length) {
      return null;
    }
...
async function searchTMDB(title, type, tmdbCache, apiKey) {
  ...
  if (clean in tmdbCache) {
    return tmdbCache[clean];
  }
...
app.get("/manifest.json", (req, res) => {
  ...
  res.json({
    ...
    description:
      "Stream your personal M3U playlist or Xtream Codes IPTV in Stremio. Auto-resolves IMDb IDs via TMDB. By Esmequiinn (reddit user Thin-Soil-4159)",
...
app.get("/:config/manifest.json", (req, res) => {
  ...
  res.json({
    ...
    description:
      "Reproduce tu lista M3U personal en Stremio con IDs IMDb automáticos. By Esmequiinn (reddit user Thin-Soil-4159)",
    ...
    if (type === "movie" && id === "m3u_movies") {
      ...
      if (search) {
        results = results.filter(m =>
          normalize(m.title).includes(search)
        );
      }
